...
proc `[]`*(self: Board, coord: Coord): PointState =
  ...
  result =
    if idx in self.b_stones:
      Black
    elif idx in self.w_stones:
      White
    else:
      Empty
...
proc remove*(board: var Board, bit_board: BitBoard) =
  assert(
    board.size == bit_board.size,
    "Attempting to remove stones from a different sized board",
  )
